...
MONITOR_SITE_NAME="${MONITOR_SITE_NAME:-Application}"
SITE_ADMIN_EMAIL="${SITE_ADMIN_EMAIL:-}"
DJANGO_DEFAULT_FROM_EMAIL="${DJANGO_DEFAULT_FROM_EMAIL:-app@localhost}"
DJANGO_EMAIL_HOST="${DJANGO_EMAIL_HOST:-}"
RELEASE_DEPLOYED_AT="${RELEASE_DEPLOYED_AT:-$(date -u +%Y-%m-%dT%H:%M:%S%z)}"
RELEASE_IMAGE_PREFIX="${RELEASE_IMAGE_PREFIX:-local/app}"
...
# Operational monitoring. The optional external profile overrides
# the notification backend and reads its secret URLs from mounted files.
DEPLOYMENT_ENVIRONMENT=production
RELEASE_REVISION=local
RELEASE_IMAGE_PREFIX=${RELEASE_IMAGE_PREFIX}
RELEASE_DEPLOYED_AT=${RELEASE_DEPLOYED_AT}
MONITOR_SITE_NAME=${MONITOR_SITE_NAME}
MONITOR_NOTIFICATION_BACKEND=email
SITE_ADMIN_EMAIL=${SITE_ADMIN_EMAIL}
...
MONITOR_OPERATIONAL_GRACE_SECONDS=600
MONITOR_BACKUP_INTERVAL_SECONDS=86400
MONITOR_BACKUP_GRACE_SECONDS=21600
MONITOR_RESTORE_INTERVAL_SECONDS=604800
MONITOR_RESTORE_GRACE_SECONDS=86400
EOF
