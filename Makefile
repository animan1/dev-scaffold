...
# Crane's Castle owns this backup artifact independently of the selected
...
build-backup-image: ## Build the website-owned backup tool image from immutable inputs
	docker build -f deploy/backup/Dockerfile \
...
verify-backup-image: build-backup-image ## Verify the exact backup image tool and user contract
	@test "$$(docker image inspect --format='{{.Config.User}}' $(BACKUP_IMAGE))" = "website-backup"
